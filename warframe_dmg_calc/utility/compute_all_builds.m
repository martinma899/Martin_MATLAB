...
num_mandatory_mods = numel(mandatory_mods_ind);
num_flexible_mods = numel(flexible_mods_ind);
...
all_remaining_mods_combs = combnk(flexible_mods_ind,num_open_mod_slots);
all_builds = [];
for i = 1:num_builds
  all_builds = [all_builds;[mandatory_mods_ind all_remaining_mods_combs(i,:)]];
end
...
for i = 1:size(all_builds,1)
  ...
  for thismod = this_build
    ...
    wp.EMT = wp.EMT+mods(thismod).EM; % total elemental multiplier
    wp.EMT = wp.EMT+mods(thismod).IM*wp.IPC; % total impact multiplier
    wp.EMT = wp.EMT+mods(thismod).PM*wp.PPC; % total puncture multiplier
    wp.EMT = wp.EMT+mods(thismod).SM*wp.SPC; % total slash multiplier
  ...
  wp.d = floor(wp.d*wp.DMT); % warframe's ridiculous round down DM multiplier algorithm
  wp.d = wp.d*wp.EMT*wp.MMT*wp.FDMT; % total modified expected per shot damage
